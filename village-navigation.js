/* The Still Becoming Village Circle — shared navigation. Exact original artwork. */
(function(){
'use strict';
if(window.__TSBVC_NAV_V4__) return;
window.__TSBVC_NAV_V4__=true;

const ART='/assets/B307A382-B6FC-4D8D-81C5-3047BDE8F4E3.png?v=nav-original';
const ITEMS=[
...
function hrefFor(item){return item[0]+'?nav=v4';}

function install(){
 if(!document.body) return false;
 if(document.getElementById('tsbvc-stable-nav')) return true;

 document.querySelectorAll('.tsbvc-site-header,.tsbvc-single-navigation,.tsbvc-nav,.destination-band,.ornament,.quick-menu,.tsbvc-quick-menu').forEach(function(x){x.remove();});
 document.querySelectorAll('header,nav').forEach(function(x){
   if(!x.closest('#tsbvc-stable-header') && !x.closest('#tsbvc-stable-nav')) x.remove();
 });

 if(!document.getElementById('tsbvc-nav-v4-style')){
   const style=document.createElement('style');
   style.id='tsbvc-nav-v4-style';
   style.textContent=`
...
`;
 ...
 h.id='tsbvc-stable-header';
 h.innerHTML='<div class="inner"><a class="brand" href="'+hrefFor(['index.html'])+'" aria-label="The Still Becoming Village Circle"><svg viewBox="0 0 60 60" aria-hidden="true"><path d="M28 54C28 36 31 18 49 7" fill="none" stroke="#5d9258" stroke-width="4" stroke-linecap="round"/><path d="M34 24C40 11 50 10 57 10c-2 10-10 16-23 16Z" fill="#82ad68"/><path d="M27 34C18 23 8 23 3 27c3 9 12 14 24 14Z" fill="#6f9e61"/></svg><span>The Still Becoming Village Circle</span></a><button type="button" aria-label="Open Village menu" aria-expanded="false"><i></i></button></div>';
 document.body.insertBefore(h,document.body.firstChild);

 const n=document.createElement('nav');
 n.id='tsbvc-stable-nav';
 ...
 ITEMS.forEach(function(item){
   ...
   a.title=item[1];
   a.style.left=(item[2]/1093.5*100)+'%';
   a.style.width=(item[3]/1093.5*100)+'%';
   if(current===item[0].toLowerCase()) a.setAttribute('aria-current','page');
   art.appendChild(a);
 });
 ...
 art.appendChild(labels);
 n.appendChild(art);
 h.insertAdjacentElement('afterend',n);

 const btn=h.querySelector('button');
 btn.addEventListener('click',function(){
   let box=document.getElementById('tsbvc-stable-menu');
   if(box){box.remove();btn.setAttribute('aria-expanded','false');return;}
   box=document.createElement('div');
   box.id='tsbvc-stable-menu';
   box.style.cssText='position:fixed;right:12px;top:82px;z-index:20000;display:flex;flex-direction:column;min-width:220px;padding:10px;background:#fff;border:1px solid #dcebe8;border-radius:18px;box-shadow:0 14px 34px rgba(22,84,91,.18)';
   ITEMS.forEach(function(item){
     const a=document.createElement('a');
     a.href=hrefFor(item);
     a.textContent=item[1];
     a.style.cssText='display:block;padding:11px 13px;color:#16545b;text-decoration:none;font:600 16px Georgia,serif;border-radius:10px';
     box.appendChild(a);
   });
