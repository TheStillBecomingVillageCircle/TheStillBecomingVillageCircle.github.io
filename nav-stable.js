...
`;
 ...
 const img=document.createElement('img');img.src=ART+'?v=stable-1';img.alt='';img.draggable=false;art.appendChild(img);
 const current=(location.pathname.split('/').pop()||'index.html').toLowerCase();
 ITEMS.forEach(function(item){const a=document.createElement('a');a.className='hot';a.href=item[0];a.setAttribute('aria-label',item[1]);a.title=item[1];a.style.left=(item[2]/1093.5*100)+'%';a.style.width=(item[3]/1093.5*100)+'%';a.style.height='78.3%';if(current===item[0].toLowerCase())a.setAttribute('aria-current','page');art.appendChild(a);});
 const labels=document.createElement('div');labels.className='labels';
 ITEMS.forEach(function(item){const a=document.createElement('a');a.href=item[0];a.textContent=item[1];a.setAttribute('aria-label',item[1]);if(current===item[0].toLowerCase())a.setAttribute('aria-current','page');labels.appendChild(a);});
 art.appendChild(labels);n.appendChild(art);h.insertAdjacentElement('afterend',n);
 const btn=h.querySelector('button');btn.addEventListener('click',function(){let box=document.getElementById('tsbvc-stable-menu');if(box){box.remove();btn.setAttribute('aria-expanded','false');return;}box=document.createElement('div');box.id='tsbvc-stable-menu';box.style.cssText='position:fixed;right:12px;top:82px;z-index:20000;display:flex;flex-direction:column;min-width:220px;padding:10px;background:#fff;border:1px solid #dcebe8;border-radius:18px;box-shadow:0 14px 34px rgba(22,84,91,.18)';ITEMS.forEach(function(item){const a=document.createElement('a');a.href=item[0];a.textContent=item[1];a.style.cssText='display:block;padding:11px 13px;color:#16545b;text-decoration:none;font:600 16px Georgia,serif;border-radius:10px';box.appendChild(a);});document.body.appendChild(box);btn.setAttribute('aria-expanded','true');});
}
