/* Stable shared Village navigation v3. Exact artwork, six destinations, cache-safe loader. */
(function(){
'use strict';
if(window.__TSBVC_STABLE_NAV_V3__) return;
window.__TSBVC_STABLE_NAV_V3__=true;
const ART='/assets/B307A382-B6FC-4D8D-81C5-3047BDE8F4E3.png';
const NAV_VERSION='stable-3';
const ITEMS=[
...
function install(){
 ...
 style.id='tsbvc-stable-nav-v3-style';
 style.textContent=`
...
`;
 ...
 ITEMS.forEach(function(item){const a=document.createElement('a');a.className='hot';a.href=hrefFor(item);a.setAttribute('aria-label',item[1]);a.title=item[1];a.style.left=(item[2]/1093.5*100)+'%';a.style.width=(item[3]/1093.5*100)+'%';if(current===item[0].toLowerCase())a.setAttribute('aria-current','page');art.appendChild(a);});
